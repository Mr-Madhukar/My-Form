...
function BuildCardPlaceholder() {
  return (
    <div className="mt-6 border border-white/4 bg-[#0c0c0c]/80 rounded-xl p-3 flex flex-col md:flex-row gap-3 min-h-35 text-left">
      {/* Pane 1: Palette */}
      <div className="w-full md:w-1/3 flex flex-col gap-1 border-b md:border-b-0 md:border-r border-white/4 pb-2 md:pb-0 md:pr-2.5">
        <span className="font-mono text-[7px] text-zinc-400 uppercase tracking-wider mb-1 block">
          Palette
        </span>
        {[
          ...
          return (
            <div
              key={item.label}
              className="flex items-center gap-1.5 rounded-lg border border-white/4 bg-white/1 px-2 py-1 text-[8px] text-zinc-400"
            >
      ...
      <div className="flex-1 flex flex-col gap-1.5 justify-center">
        <span className="font-mono text-[7px] text-zinc-400 uppercase tracking-wider mb-0.5">
          Canvas Workspace
        </span>
        ...
        <p className="text-[7.5px] text-zinc-400 font-mono">explicit_save: CMD+S active</p>
      </div>
    </div>
  );
}
...
function RunnerCardPlaceholder() {
  return (
    <div className="mt-6 border border-white/4 bg-[#0c0c0c]/90 rounded-xl p-3.5 flex flex-col justify-between min-h-35 text-left">
      <div className="flex justify-between items-center text-[7px] text-zinc-400 font-mono border-b border-white/4 pb-1.5">
        <span>myform.app/f/onboarding</span>
        <span className="text-[#E8854A] font-semibold bg-[#E8854A]/10 px-1.5 py-0.5 rounded-full">
          92% Done
        </span>
      </div>
      <div className="py-2.5 space-y-1">
        <p className="text-[8px] text-zinc-400 font-mono">Question 2 of 3</p>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="rounded-lg border border-white/6 bg-white/1 px-2.5 py-1.5 text-[8px] text-zinc-400">
          Retention
        </span>
      </div>
    </div>
...
function FollowUpCardPlaceholder() {
  return (
    <div className="mt-6 space-y-3.5 text-left min-h-35 flex flex-col justify-center">
      {/* User Answer */}
      <div className="rounded-xl border border-white/5 bg-white/1 p-2.5 relative">
        <p className="text-[7px] font-mono text-zinc-400 uppercase tracking-widest leading-none mb-1.5">
          Open-Text Answer
        </p>
        <p className="text-[9.5px] text-white">&ldquo;We need to collect leads on our landing page.&rdquo;</p>
      </div>
      {/* AI Follow-Up node */}
      <div className="rounded-xl border-l-2 border-[#E8854A] bg-[#E8854A]/4 p-2.5 relative shadow-md">
        ...
        <p className="text-[9.5px] leading-relaxed text-zinc-300">
          &ldquo;What specific lead information is critical besides emails?&rdquo;
        </p>
      </div>
    </div>
  );
...
function GeneratorCardPlaceholder() {
  return (
    <div className="mt-6 space-y-3 text-left min-h-35 flex flex-col justify-center">
      {/* User Prompt */}
      <div className="rounded-xl border border-dashed border-white/8 p-2.5 bg-white/1">
        <p className="text-[7px] font-mono text-zinc-400 uppercase tracking-widest leading-none mb-1.5">
          AI Prompt
        </p>
        <p className="text-[9.5px] text-[#E8854A] font-semibold">
          &ldquo;Create a customer checkout survey&rdquo;
        </p>
      </div>
      {/* Compiling schemas */}
      <div className="flex gap-2">
        {["1. Rating", "2. Choice"].map((step) => (
          <div
            key={step}
            className="flex-1 rounded-lg border border-white/4 bg-[#0c0c0c] px-2 py-1.5 text-[8px] font-mono text-zinc-400 flex items-center justify-between"
          >
            <span>{step}</span>
...
function AnalyticsCardPlaceholder() {
  return (
    <div className="mt-6 space-y-2.5 text-left min-h-35 flex flex-col justify-center">
      <div className="flex justify-between items-center text-[8px] font-mono text-zinc-400 leading-none mb-1">
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={bar.label} className="space-y-0.5">
            ...
            <div className="h-1.5 w-full bg-white/3 rounded-full overflow-hidden p-px border border-white/3">
...
function MockBentoCard({ card }: { readonly card: BentoCard }) {
  ...
  return (
    ...
    >
      ...
      {/* Inner card core */}
      <div className="relative min-h-56 overflow-hidden rounded-[1.4rem] bg-[#111] p-5 shadow-[inset_0_1px_1px_rgba(255,255,255,0.06)] border border-white/2 flex flex-col justify-between">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[15px] font-semibold tracking-tight text-[#F2F2F2]">{card.title}</p>
            <p className="mt-0.5 font-mono text-[9px] text-zinc-400 leading-none uppercase tracking-wide">
              {card.meta}
            </p>
          </div>
