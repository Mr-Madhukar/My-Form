...
export function LandingButton({
  ...
  if (variant === "primary") {
    return (
      ...
      >
        {/* Outward glow */}
        <div className="absolute inset-0 rounded-full bg-linear-to-r from-[#E8854A] via-[#ffaa75] to-[#E8854A] opacity-60 blur-[3px]" />
        <Link
          href={href}
        ...
        >
          {/* Shimmer sweep */}
          <div className="absolute inset-y-0 -left-full w-1/2 bg-linear-to-r from-transparent via-white/30 to-transparent skew-x-30 transition-all duration-1000 ease-out group-hover/btn:left-[150%]" />
  ...
  return (
    ...
    >
      <Link
        href={href}
        className={`flex items-center justify-center rounded-full border border-white/8 bg-white/3 font-semibold text-[#F2F2F2] transition-all duration-300 hover:border-white/14 hover:bg-white/6 group/btn ${sizeClasses[size]} ${fullWidth ? "w-full" : ""}`}
      >
        {children}
