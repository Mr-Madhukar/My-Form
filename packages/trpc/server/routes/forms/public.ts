...
export const formsPublicRouter = router({
  ...
  submit: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (publishedSettings.expiresAt) {
        const expiryDate = new Date(publishedSettings.expiresAt as string);
        if (!isNaN(expiryDate.getTime()) && new Date() > expiryDate) {
          throw new TRPCError({ code: "FORBIDDEN", message: "form_expired" });
        }
      }

      // Check max response limit
      if (publishedSettings.maxResponses) {
        const maxResponses = Number(publishedSettings.maxResponses);
